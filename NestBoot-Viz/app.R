...
server <- function(input, output) {
  ...
  output$overlapPLOT <- renderPlotly({
    ...
    }else if(identical(input$data,"tlsco")){
      return()}else{validate(
        need(input$raw == FALSE, "cannot (yet) analyze uploaded GRN overlap"))
        # inFile <- input$file2
        # if (is.null(inFile))
        #   return(NULL)
...
ui <- shinyUI(fluidPage(#theme = shinytheme("united"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      h4("networks from:"),h5("'Perturbation-based gene regulatory network inference to unravel oncogenic mechanisms'"),
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("CytoscapeJS",h4("This tab displays the seleceted GRN where blue links reflect up regulation while red reflect negative down regulation."),tags$hr(),selectInput("clay",label="Layout:",c("CoSE"="cose","Cola"="cola","Concentric"="concentric","Circle"="circle","Dagre"="dagre","Grid"="grid","arbor"="arbor","markov"="cytoscape-markov-cluster")),downloadButton('downloadPlot', 'Download'),rcytoscapejsOutput("CytoscapeJS",height='800px')),
